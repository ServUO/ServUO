...
namespace Ultima
{
    ...
    {
        private readonly RadarCol _RadarCol;
        ...
        public static Map Felucca(UltimaOnlineReaderFactory factory)
        {
            return new Map(factory, 0, 0, 6144, 4096);
        }

        public static Map Trammel(UltimaOnlineReaderFactory factory)
        {

            return new Map(factory, 0, 1, 6144, 4096);

        }
        public static Map Ilshenar(UltimaOnlineReaderFactory factory)
        {
            return new Map(factory, 2, 2, 2304, 1600);
        }
        public static Map Malas(UltimaOnlineReaderFactory factory)
        {
            return new Map(factory, 3, 3, 2560, 2048);
        }
        public static Map Tokuno(UltimaOnlineReaderFactory factory)
        {
            return new Map(factory, 4, 4, 1448, 1448);
        }

        public static Map TerMur(UltimaOnlineReaderFactory factory)
        {
            return new Map(factory, 5, 5, 1280, 4096);
        }
        ...
        public Map(UltimaOnlineReaderFactory factory, int fileIndex, int mapId, int width, int height)
            : this(
                fileIndex, mapId, width, height, factory.RadarCol, factory.TileData, factory.Hues, factory.Files,
                factory.Art)
        {
            Factory = factory;
        }

        public Map(UltimaOnlineReaderFactory factory, string path, int fileIndex, int mapID, int width, int height)
        : this(
            path, fileIndex, mapID, width, height, factory.RadarCol, factory.TileData, factory.Hues, factory.Files,
            factory.Art)
        ...
        public void StartUpSetDiff(bool value)
        {
            m_UseDiff = value;
        }

        public Map(int fileIndex, int mapID, int width, int height, RadarCol radarCol, TileData tileData, Hues hues, Files files, Art art)
        {
        ...
        public TileMatrix Tiles
        {
            get
            {
                if (m_Tiles == null)
                {
                    m_Tiles = new TileMatrix(m_FileIndex, m_MapID, m_Width, m_Height, m_path, this, _Art, _files);
                }
        ...
        private unsafe short[] RenderBlock(int x, int y, bool drawStatics, bool diff)
        {
            ...
            fixed (short* pColors = _RadarCol.Colors)
            {
                fixed (int* pHeight = _TileData.HeightTable)
                {
                            ...
                            if (drawStatics)
                            {
                                ...
                                for (int k = 0, v = 0; k < 8; ++k, v += 8)
                                {
                                    for (int p = 0; p < 8; ++p)
                                    {
                                        ...
                                        {
                                            *pvData++ = _Hues.GetHue(highHue - 1).Colors[(pColors[highID + 0x4000] >> 10) & 0x1F];
                                        }
        ...
        {
            string indexPath = _files.GetFilePath("staidx{0}.mul", map.FileIndex);
            FileStream m_Index;
            ...
            string staticsPath = _files.GetFilePath("statics{0}.mul", map.FileIndex);
                                ...
                                else
                                {
                                    ...
                                    if (!remove) //without duplicate remove
                                    {
                                        ...
                                        for (int i = 0; i < count; ++i)
                                        {
                                            ...
                                            if ((graphic >= 0) && (graphic <= _Art.GetMaxItemID()))
                                            {
                                                if (shue < 0)
                                                {
                                                    shue = 0;
                                        ...
                                        if (tilelist != null)
                                        {
                                            ...
                                            {
                                                if ((tilelist[i].m_ID >= 0) && (tilelist[i].m_ID <= _Art.GetMaxItemID()))
                                                {
                                    ...
                                    else //with duplicate remove
                                    {
                                        ...
                                        for (int i = 0; i < count; ++i)
                                        {
                                            ...
                                            if ((tile.m_ID >= 0) && (tile.m_ID <= _Art.GetMaxItemID()))
                                            {
                                                if (tile.m_Hue < 0)
                                                {
                                                    tile.m_Hue = 0;
                                        ...
                                        if (map.Tiles.PendingStatic(x, y))
                                        {
                                            ...
                                            {
                                                if ((pending[i].m_ID >= 0) && (pending[i].m_ID <= _Art.GetMaxItemID()))
                                                {
        ...
        public void RewriteMap(string path, int map, int width, int height)
        {
            string mapPath = _files.GetFilePath("map{0}.mul", map);
            FileStream m_map;
                ...
                for (int x = 0; x < m_Width; ++x)
                {
                    for (int y = 0; y < m_Height; ++y)
                    {
                        ...
                        foreach (HuedTile currstatic in Tiles.GetStaticTiles(x, y))
                        {
                            ...
                            {
                                if (_TileData.ItemTable[currstatic.ID].Height + currstatic.Z < currtile.Z)
                                {
        ...
        public void ReportInvalidMapIDs(string reportfile)
        {
                    ...
                    new FileStream(reportfile, FileMode.Create, FileAccess.ReadWrite), Encoding.GetEncoding(1252)))
            {
                ...
                for (int x = 0; x < m_Width; ++x)
                {
                    ...
                    {
                        Tile currtile = Tiles.GetLandTile(x, y);
                        if (!_Art.IsValidLand(currtile.ID))
                        {
