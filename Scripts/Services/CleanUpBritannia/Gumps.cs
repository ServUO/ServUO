...
namespace Server.Engines.CleanUpBritannia
{
	public class CleanUpBritanniaRewardGump : Gump
	{
		...
		{
            user.CloseGump(typeof(CleanUpBritanniaRewardGump));
			...
			AddHtmlLocalized(70, 35, 270, 20, 1151316, 0x1, false, false); // Clean Up Britannia
			...
			while (CleanUpBritanniaRewards.Rewards != null && Index < CleanUpBritanniaRewards.Rewards.Count)
                DisplayRewardPage();
		}
		...
		{
            AddPage(Page);
			double points = PointsSystem.CleanUpBritannia.GetPoints(User);
			
			int offset = 110;
            int next = 0;
			int max = GetMax(CleanUpBritanniaRewards.Rewards);
			
			while (offset + next < 300 && Index < CleanUpBritanniaRewards.Rewards.Count)
			{
				CollectionItem item = CleanUpBritanniaRewards.Rewards[Index];
                ...
                if (Index < CleanUpBritanniaRewards.Rewards.Count)
                    next = Math.Max(CleanUpBritanniaRewards.Rewards[Index].Height, 20);
                else
                    next = 0;
			}
		...
		public override void OnResponse(Server.Network.NetState state, RelayInfo info)
		{
			...
			{
				CollectionItem item = CleanUpBritanniaRewards.Rewards[info.ButtonID - 200];
				double points = PointsSystem.CleanUpBritannia.GetPoints(from);
