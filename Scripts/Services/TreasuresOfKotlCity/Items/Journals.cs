...
namespace Server.Engines.TreasuresOfKotlCity
{
    ...
    {
        public JournalGump(TextDefinition title, TextDefinition body)
            : base(10, 10)
        {
            AddImage(0, 0, 0x761C);
            ...
            if (title.Number > 0)
            {
                AddHtmlLocalized(50, 20, 450, 20, 1154645, String.Format("#{0}", title.Number.ToString()), 0, false, false);
            }
            else
            {
                AddHtml(50, 20, 450, 20, String.Format("<CENTER>{0}</CENTER>", title.String), false, false);
            }

            if (body.Number > 0)
            {
                AddHtmlLocalized(95, 50, 380, 600, body.Number, 1, false, true);
            }
            else
            {
                AddHtml(95, 50, 380, 600, body.String, false, true);
            }
        }
    }
    ...
    public abstract class BaseJournal : Item
    {
        ...
        public override void OnDoubleClick(Mobile m)
        {
            ...
            {
                m.SendGump(new JournalGump(Title, Body));
            }
        }
