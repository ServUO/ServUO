...
namespace Server
{
    public class LootPack
    {
        ...
        public void Generate(Mobile from, Container cont, bool spawning, int luckChance)
        {
            ...
            {
                LootPackEntry entry = this.m_Entries[i];
        ...
        public static LootPack Poor
        {
            get
            {
                return Core.SE ? SePoor : Core.AOS ? AosPoor : OldPoor;
            }
        }
        public static LootPack Meager
        {
            get
            {
                return Core.SE ? SeMeager : Core.AOS ? AosMeager : OldMeager;
            }
        }
        public static LootPack Average
        {
            get
            {
                return Core.SE ? SeAverage : Core.AOS ? AosAverage : OldAverage;
            }
        }
        public static LootPack Rich
        {
            get
            {
                return Core.SE ? SeRich : Core.AOS ? AosRich : OldRich;
            }
        }
        ...
        public static LootPack UltraRich
        {
            get
            {
                return Core.SE ? SeUltraRich : Core.AOS ? AosUltraRich : OldUltraRich;
            }
        }
        public static LootPack SuperBoss
        {
            get
            {
                return Core.SE ? SeSuperBoss : Core.AOS ? AosSuperBoss : OldSuperBoss;
            }
        }
        #endregion
    ...
    public class LootPackEntry
    {
        ...
        public int MaxProps
        {
            get
            {
                return this.m_MaxProps;
            }
            set
            {
                this.m_MaxProps = value;
            }
        }
        ...
        {
            if (this.m_AtSpawnTime != spawning)
                return null;

            int totalChance = 0;

            for (int i = 0; i < this.m_Items.Length; ++i)
                totalChance += this.m_Items[i].Chance;

            int rnd = Utility.Random(totalChance);

            for (int i = 0; i < this.m_Items.Length; ++i)
            {
                LootPackItem item = this.m_Items[i];

                if (rnd < item.Chance)
                    return this.Mutate(from, luckChance, item.Construct(IsInTokuno(from), IsMondain(from)));
        ...
        {
            int rnd = Utility.RandomMinMax(this.m_MinIntensity, this.m_MaxIntensity);
        ...
        {
            this.m_AtSpawnTime = atSpawnTime;
            this.m_Items = items;
            this.m_Chance = (int)(100 * chance);
            this.m_Quantity = quantity;
            this.m_MaxProps = maxProps;
    ...
    public class LootPackItem
    {
        ...
        public Item Construct(bool inTokuno, bool isMondain)
        {
            try
            {
                ...
                if (this.m_Type == typeof(BaseRanged))
                    item = Loot.RandomRangedWeapon(inTokuno, isMondain);
                else if (this.m_Type == typeof(BaseWeapon))
                    item = Loot.RandomWeapon(inTokuno, isMondain);
                else if (this.m_Type == typeof(BaseArmor))
                    item = Loot.RandomArmorOrHat(inTokuno, isMondain);
                else if (this.m_Type == typeof(BaseShield))
                    item = Loot.RandomShield();
                else if (this.m_Type == typeof(BaseJewel))
                    item = Core.AOS ? Loot.RandomJewelry() : Loot.RandomArmorOrShieldOrWeapon();
                else if (this.m_Type == typeof(BaseInstrument))
                    item = Loot.RandomInstrument();
                else if (this.m_Type == typeof(Amber)) // gem
                    item = Loot.RandomGem();
                else if (this.m_Type == typeof(ClumsyScroll)) // low scroll
                    item = RandomScroll(0, 1, 3);
                else if (this.m_Type == typeof(ArchCureScroll)) // med scroll
                    item = RandomScroll(1, 4, 7);
                else if (this.m_Type == typeof(SummonAirElementalScroll)) // high scroll
                    item = RandomScroll(2, 8, 8);
                else
                    item = Activator.CreateInstance(this.m_Type) as Item;
    ...
    public class LootPackDice
    {
        ...
        public int Roll()
        {
            ...
            for (int i = 0; i < this.m_Count; ++i)
                v += Utility.Random(1, this.m_Sides);
        ...
        public LootPackDice(string str)
        {
            ...
            this.m_Count = Utility.ToInt32(str.Substring(start, index - start));
            ...
            this.m_Sides = Utility.ToInt32(str.Substring(start, index - start));
            ...
            this.m_Bonus = Utility.ToInt32(str.Substring(start, index - start));
