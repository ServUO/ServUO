...
{
    public class RatLord : BaseChampion
    {
        [Constructable]
        public RatLord()
            : base(AIType.AI_Melee)
        {
            ...
            this.SetMana(150, 225);

            this.SetDamage(18, 27);
        ...
        public void DoSpecialAbility(Mobile target)
        {
            if (target == null || target.Deleted) //sanity
                return;
            if (0.0 >= Utility.RandomDouble()) // 0% chance to polymorph attacker into a ratman
                this.Polymorph(target);

            if (0.1 >= Utility.RandomDouble()) // 10% chance to more ratmen
                this.SpawnRatmen(target);

            if (0.00 >= Utility.RandomDouble() && !this.IsBodyMod) // 0% chance to polymorph into a ratman
                this.Polymorph(this);
        }
