...
namespace Server.Multis
{
    public class BoatPlacementGump : Gump
    {
        ...
        private NewBaseBoatDeed _deed;
		private BaseDockedNewBoat _dockedBoat;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            if (info.ButtonID == 1)
			{
				...
				else if (_dockedBoat != null)
				{
					_dockedBoat.PlacementDirection(_from, Direction.West);
				}
			}
			else if (info.ButtonID == 2)
			{
				...
				{
					_dockedBoat.PlacementDirection(_from, Direction.North);
				}
			}
            else if (info.ButtonID == 3)
			{
				...
				{
					_dockedBoat.PlacementDirection(_from, Direction.South);
				}
			}
            else if (info.ButtonID == 4)
			{
				...
				{
					_dockedBoat.PlacementDirection(_from, Direction.East);
				}
			}
        }
    }
