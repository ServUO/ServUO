...
namespace Server.Network
{
	...
	public sealed class DamagePacket : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, e, amount));
		}
	...
	public sealed class CancelArrow : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, p));
		}
	...
	public sealed class SecureTradeEquip : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, item, m));
		}
	...
	public sealed class VendorBuyContent : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, list));
		}
	...
	public sealed class DisplayBuyList : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, vendor));
		}
	...
	public sealed class TripTimeResponse : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, ping, unk));
		}
	...
	public sealed class SpeedControl : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, speed));
		}
	...
	public sealed class GlobalLightLevel : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, level));
		}
	...
	public sealed class DisplayContextMenu : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, menu));
		}
			...
			: base(0xBF)
		{
			...
			if (retro)
			{
				...
				{
					if (e.Number < 3000000 || e.Number > 3032767)
					{
						retro = false;
					...
					if ((e.Color & 0xFFFF) != 0xFFFF || (e.Flags & CMEFlags.Colored) != 0)
					{
						size += 2;
					}
				}
			...
			for (var i = 0; i < length; ++i)
			{
				...
				if (retro && (e.Color & 0xFFFF) != 0xFFFF)
				{
	...
	public sealed class EquipUpdate : Packet
	{
			...
			: base(0x2E, 15)
		{
			...
			if (item.Parent is Mobile mp)
			{
				if (mp.SolidHueOverride >= 0)
				{
					hue = mp.SolidHueOverride;
				}
			}
	...
	public sealed class WorldItem : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, item));
		}
	...
	public sealed class LiftRej : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, reason));
		}
	...
	public sealed class PlayerMove : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, dir));
		}
	...
	public sealed class ScreenEffect : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, effect));
		}
	...
	public sealed class DisplaySpellbook : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, book));
		}
	...
	public sealed class SpellbookContent : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, item, offset, content));
		}
	...
	public sealed class ContainerDisplay : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, c));
		}
		...
		private ContainerDisplay(Container c, int? unk)
			: base(0x24, unk != null ? 9 : 7)
		{
	...
	public sealed class MobileMoving : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, moving));
		}
		...
		public static MobileMoving Instantiate(NetState ns, Mobile moving)
		{
			...
			var noto = Math.Max(0, Math.Min(7, Notoriety.Compute(ns.Mobile, moving)));
	...
	public sealed class MultiTargetReq : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, t));
		}
	...
	public sealed class PopupMessage : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, msg));
		}
	...
	public sealed class PlayMusic : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, name));
		}
	...
	public sealed class SeasonChange : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, playSound));
		}

		public static SeasonChange Instantiate(NetState ns, bool playSound)
		{
			var season = Math.Max(0, Math.Min(5, ns.Mobile?.GetSeason() ?? 0));

			var hash = (season << 16) | (playSound ? 1 : 0);

			return PacketCache<SeasonChange>.Acquire(hash, () => new SeasonChange(season, playSound));
		}
	...
	public sealed class MobileStatus : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, beheld));
		}
	...
	public sealed class PingAck : Packet
	{
		...
		{
			return Send(ns, Instantiate(ns, ping));
		}
	...
	{
		private static readonly ThreadLocal<P> m_Instance = new ThreadLocal<P>();

		private static volatile ConcurrentDictionary<object, P> m_Instances;

		public static P Global(Func<P> ctor)
		{
			var p = m_Instance.Value;

			if (p == null || (p.State & PacketState.Static) == 0)
			{
				m_Instance.Value = p = Packet.SetStatic(ctor());
			}
		...
		public static P Acquire<K>(K key, Func<P> ctor)
		{
			if (m_Instances == null)
			{
				m_Instances = new ConcurrentDictionary<object, P>();
			}

			m_Instances.TryGetValue(key, out var p);
		...
		public static void Free()
		{
			...
			if (p != null)
			{
				...
				m_Instance.Value = default(P);
			}
		}

		public static void Free<K>(K key)
		{
			if (m_Instances != null && m_Instances.TryRemove(key, out var p))
			{
				Packet.Release(ref p);
			}
		}
		...
		public static void Clear()
		{
			...
			if (m_Instances != null)
			{
				foreach (var p in m_Instances.Values)
				{
					Packet.Release(p);
				}
